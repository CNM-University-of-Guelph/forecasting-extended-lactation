...
parity_fit_stats_formatted <-
  parity_fit_stats %>% 
   select(Model, Parity_class, starts_with(c("R2","MAE", "RMSPE", "CCC", "r_","Cb", "DW"))) %>% 
...
laclen_fit_stats_formatted <-
  laclen_fit_stats %>% 
   select(Model, Lac_len_group, starts_with(c("R2","MAE", "RMSPE","CCC", "r_","Cb", "DW"))) %>% 
  mutate(across(where(is.double), ~round(.x,2))) %>% 
...
```{r}
worst_fits <- combined_fit_stats %>% group_by(ID) %>% summarise(mean_MAE = mean(MAE, na.rm=TRUE)) %>%  slice_max(order_by = mean_MAE, n = 6) %>% arrange(desc(mean_MAE))

worst_fits_ID <- worst_fits %>% pull(ID) %>% unique()

best_fits <- combined_fit_stats %>% group_by(ID) %>% summarise(mean_MAE = mean(MAE, na.rm=TRUE)) %>%  slice_min(order_by = mean_MAE, n = 6) %>% arrange(mean_MAE)
...
(p_best_fits_a <- 
  f_plot_individual_three_models(
    combined_fit_stats,
    combined_preds_fitted, 
    best_fits_ID, 
    y_label_loc = 50,
...
working_subset <- 
  left_join(
  combined_fit_stats %>% select(ID, Parity, Parity_class, Model, RMSPE, MAE),
  combined_params %>% select(ID, Parity, Model, Mp, Persistency)
)


cor_tables_GOF_Mp_Persistency <- 
working_subset %>% 
  group_by(Model) %>% 
  summarise(cor_RMSPEvsMp = cor(RMSPE, Mp, use = 'complete.obs'),
            cor_RMSPEvsPersist = cor(RMSPE, Persistency, use = 'complete.obs'),
            cor_MAEvsMp = cor(MAE, Mp, use = 'complete.obs'),
            cor_MAEvsPersist = cor(MAE, Persistency, use = 'complete.obs'))
  
cor_tables_GOF_Mp_Persistency
```

```{r}
data.table::fwrite(cor_tables_GOF_Mp_Persistency, "./Outputs/cor_tables_GOF_Mp_Persistency.csv")
```
